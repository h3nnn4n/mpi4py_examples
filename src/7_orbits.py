...
class Planet:
    ...
    def acceleration(self, state, unused_t):
        ...
        for p in g_listOfPlanets:
            # if rank == 0: print(g_listOfPlanets)
...
def master():
    ...
    while True:
        ...
        t, dt = comm.bcast((t, dt))
        g_listOfPlanets = comm.allgather(g_listOfPlanets[rank])
...
if rank == 0:
    ...
    g_listOfPlanets = comm.bcast(g_listOfPlanets)

    while True:
        t, dt = comm.bcast(None)
        g_listOfPlanets = comm.allgather(g_listOfPlanets[rank])

        if not (g_listOfPlanets[rank]._merged):
            g_listOfPlanets[rank].updatePlanet(t, dt)
